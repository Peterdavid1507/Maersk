    ...
    <h3>Welcome <asp:Label runat="server" Font-Bold="true" ForeColor="#ae0000" ID="welcome"></asp:Label></h3>
        <asp:GridView ID="gvViewShipping" CssClass="table table-striped table-bordered table-condensed" runat="server" AllowPaging="True" AutoGenerateColumns="False" DataKeyNames="shipping_id" DataSourceID="dsCheckShipping" >
            <Columns>
                ...
                <asp:BoundField DataField="shipping_remarks" HeaderText="shipping_remarks" SortExpression="shipping_remarks" />
                ...
                <asp:TemplateField>
                    <ItemTemplate>
                        <asp:HyperLink ID="hlEdit" runat="server" Text="Edit"
                            NavigateUrl='<%# "EditShipping.aspx?id=" + Eval("shipping_id") + "&departure=" + Server.UrlEncode(Eval("departure").ToString())%>' /> 
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField>
                    <ItemTemplate>
        ...
        <asp:SqlDataSource ID="dsCheckShipping" runat="server" ConnectionString="<%$ ConnectionStrings:DefaultConnection %>" 
            SelectCommand="SELECT shipping_id, p1.port_name AS departure, p2.port_name AS arrival, shipping_date, shipping_status, shipping_cost, shipping_remarks, c.container_name AS ContainerName, c.container_size AS ContainerSize
            FROM shipping s
